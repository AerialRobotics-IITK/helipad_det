...
int isSimilar(std::vector<double>& signature, std::vector<cv::Point>& contour, double a, double b, double c, double d, double tolerance){
	...
	int start_index=-1, zero_length=0;
	...
	int c1=start_index,c2,c4,c5,c7,c8,c10,c11;
	...
	for(int i=0;i<signature_size;i++){
		...
		if(count_segments==4)
			c4 = (i+start_index)%signature_size;

		if(count_segments==5)
			c5 = (i+start_index)%signature_size;
		...
		if(count_segments==10)
			c10 = (i+start_index)%signature_size;

		if(count_segments==11)
			c11 = (i+start_index)%signature_size;
	...
	if(fabs((m1 - m2)/(1 + m1*m2)) > tan(30.0*CV_PI/180))	return 0;

	m1 = (contour.at(c4).y - contour.at(c5).y)*1.0/(contour.at(c4).x - contour.at(c5).x);
	m2 = (contour.at(c10).y - contour.at(c11).y)*1.0/(contour.at(c10).x - contour.at(c11).x);
